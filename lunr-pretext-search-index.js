...
},
{
  ...
  "number": "1.1.3.3",
  "title": "",
  "body": "  Create a new paragraph for text using the button at the top toolbar:   In this new paragraph, type in the following information, hitting enter for a new line between each part.     Your first and last name.      Your student number.      The name of your instructor.      The title of the activity.    Ctrl+Shift+J and Ctrl+Shift+K are useful shortcuts for creating paragraphs after or before the current line.    "
},
{
...
{
  "id": "act_creating_a_maple_worksheet-4-5",
  "level": "2",
  "url": "act_creating_a_maple_worksheet.html#act_creating_a_maple_worksheet-4-5",
  "type": "Exercise",
  "number": "1.1.3.5",
  "title": "",
...
},
{
  ...
  "number": "1.1.3.9",
  "title": "",
  "body": "  Evaluate each of the expressions on a new execution group.        The mathematical constant Pi must be typed into Maple as capitalised Pi . Never type pi , unless you only wish to display the Greek letter without its numerical value.              The exp(x) mathematical functions exponential function is the exponential function, . Never type using the 'e' key on the keyboard, as Maple will not recognize it as the exponential function. However, you can find and the function in the palettes toolbar.         "
},
{
  "id": "ex-evalf",
  "level": "2",
  "url": "act_creating_a_maple_worksheet.html#ex-evalf",
  "type": "Exercise",
  "number": "1.1.3.10",
  "title": "",
  "body": "  On each of the four Maple inputs you created in , add a semicolon at the end of the line, followed by the command evalf(%) . For example:   cos(Pi\/6); evalf(%);   After adding the additional command to each line, run each line a second time to see two outputs: the exact form and the approximate decimal form.   Two commands at once  The semicolon ; separates commands if you want to run multiple commands on the same Maple input. The ditto operator, % will recall the last output and insert it into the desired command (in this case, evalf() .    "
},
{
...
},
{
  ...
  "number": "1.2.3.4",
  "title": "",
  "body": "  Now we would like Maple perform all three commands together.     Have Maple expand the rational expression .      Add a semicolon to the end of the line, followed by simplify(%) .      Add another semicolon to the end of the line, followed by factor(%) .      Hit Enter to run all three commands together.    It is a good practice whenever using the % ditto operator shortcut on a previous command to run both commands simultaneously on the same Maple input.     You should see three outputs now: expanding, simplifying, and factoring.   Multi-line commands  You can add a line break between commands on the same Maple input without running them with Shift+Enter.    "
},
{
  "id": "act_basics-4-5",
  "level": "2",
...
},
{
  ...
  "number": "1.4.3.1",
  "title": "",
  "body": "  Consider the graphs of and . Can we apply a transformation of one graph to obtain the other?   When plotting trigonometric functions such as and , if you do not specify the -axis interval, Maple will default to increments of from to .      Plot the graphs of and on the same set of axes. Use red for and blue for .      By what amount do you need to shift the graph of to the left (negative direction) so that it coincides with the graph of ? Answer the question in sentence form by using the   button to create a new paragraph after the current line.      By what amount do you need to shift the graph of to the riught (positive direction) so that it coincides with the graph of ? Answer the question in sentence form using a new paragraph.    "
},
{
...
},
{
  ...
  "number": "1.6.3.2",
  "title": "",
  "body": "  Limits are quite useful for determining the behavour of a function around a discontinuity.   Some versions of Maple may not display discontinutes of certain functions properly. Instead, the graph may even appear as connected with a vertical line! In this case, you may include discont=true as a parameter in the plot( ) command to fix the graph output. plot discontinuities       Assign the function using the assignment operator := .      Plot , choosing a range that clearly shows the discontinuity at .      Using three different limit() commands, evaluate left-hand limit, the right-hand limit, and the two-sided limit as approaches zero.    In Maple, the absolute value function may be typed as abs( ) .    "
},
{
  "id": "act_limits-4-3",
  "level": "2",
...
},
{
  ...
  "number": "1.7.3.1",
  "title": "",
  "body": "  Assign the function using the assignment operator := .   Some versions of Maple may not display discontinutes of certain functions properly. Instead, the graph may even appear as connected with a vertical line! In this case, you may include discont=true as a parameter in the plot( ) command to fix the graph output. plot discontinuities       Plot the function and visually try to determine the -values of any vertical asymptotes.      Evaluate the left- and right-hand limits on either side of each value from the previous step using limit() commands. If Maple has outputted or as the value of these limits, then you have correctly determined the location of a vertical asymptote.    "
},
{
...
},
{
  ...
  "number": "1.9.3.4",
  "title": "",
  "body": "  The higher derivatives of and form a predictable pattern. What if these derivatives also involve a chain rule? In this exercise, you'll look for a pattern of derivatives in the function .   The th derivative of the function can be computed by using the diff() command or ' notation. See for more information. You can also make use of the Calculus palette: derivative diff!higher derivatives  derivative prime notation!higher derivatives        Assign the function . mathematical functions sine     To type the mathematical constant , be sure to use Pi . Pi     Don't forget to include multiplication between and .      Use Maple to find the first, second, third, and fourth derivative of . You should notice a pattern. Try to describe this pattern in a paragraph. derivative diff!higher derivatives       Using your this pattern, try to predict the 77th derivative of . Then, evaluate the 77th derivative directly using a single diff() command. derivative diff!higher derivatives       (Optional) Use the examples in to write a loop that will output the first derivatives of . loops     "
},
{
...
},
{
  ...
  "number": "1.14.3.2",
  "title": "",
  "body": "  In the last exercise, you used Newton's Method to numerically approxiamte the values of the two roots, . In this example, you will use Newton's method to numerically approximate      Assign a function that has a known root of       Apply Newton's method to the function from part (a) with an initial value for that is close to . Then, evaluate to digits using evalf() and verify that the values agree.    "
},
{
...
},
{
  ...
  "number": "1.16.3.2",
  "title": "",
  "body": "  Now you are ready to apply Rolle's Theorem (and a bit of reasoning) to determine if there are any other positive integers and satisfy equation . There are no calculations to perform in this exercise. Instead, you will need to adequately explain your answers in paragraph form.  Let's begin by assuming that .     If and satisfy equation , then . From what Rolle's Theorem states, what does this tell you about the location of a critical value of where ?      Since you have already found the critical value(s) of , can there be any other positive integers and such that ?    "
},
{
  "id": "act_sweet_16-4-3",
  "level": "2",
...
},
{
  ...
  "number": "2.7",
  "title": "",
  "body": "  Sketch the plot of and state the equations of vertical and horizontal asymptotes. asymptote     Vertical asymptote: ; Horizontal asymptotes:    "
},
{
...
{
  "id": "act_visualizing_the_net_change_theorem-4-2",
  "level": "2",
  "url": "act_visualizing_the_net_change_theorem.html#act_visualizing_the_net_change_theorem-4-2",
  "type": "Exercise",
  "number": "3.2.3.2",
  "title": "",
...
{
  "id": "act_visualizing_the_net_change_theorem-4-3",
  "level": "2",
  "url": "act_visualizing_the_net_change_theorem.html#act_visualizing_the_net_change_theorem-4-3",
  "type": "Exercise",
  "number": "3.2.3.3",
  "title": "",
  "body": "  In this exercise, you will determine the exact value of the net change of on the interval , using a limit of Riemann sums as well as the convnenient Int() (or int() ) command.     Use the ApproximateInt() command with method=right and partitions to give the Riemann sum for on the interval . Then, use the limit() command to find the limit of this value as goes to infinity. limit     The ApproximateInt() command will likely only output the net change in summation notation. You may need to force Maple to convert the summation to a closed form before evaluating the limit. To do this, follow up your ApproximateInt() output with a value(%) command before using the limit() command. The ditto operator % is a shortcut that reuses the most recent output. value  ditto operator       Compute by using the Int() command. Verify that this value matches the limit of the Riemann sum in the previous part.    You may use the inert Int() command followed by value(%) (this is typically done on the same input line, with a semicolon between commands) or the int() command, which evaluates immediately.    "
},
{
  "id": "act_visualizing_the_net_change_theorem-4-4",
  "level": "2",
  "url": "act_visualizing_the_net_change_theorem.html#act_visualizing_the_net_change_theorem-4-4",
  "type": "Exercise",
  "number": "3.2.3.4",
  "title": "",
...
{
  "id": "act_visualizing_the_net_change_theorem-4-5",
  "level": "2",
  "url": "act_visualizing_the_net_change_theorem.html#act_visualizing_the_net_change_theorem-4-5",
  "type": "Exercise",
  "number": "3.2.3.5",
  "title": "",
  "body": "  In this exercise, you will determine the exact value of the net change of on the interval , using a limit of Riemann sums as well as the convnenient Int() (or int() ) command.     Use the ApproximateInt() command with method=right and partitions to give the Riemann sum for on the interval . Then, use the limit() command to find the limit of this value as goes to infinity. limit     Once again, the ApproximateInt() command will likely only output the net change in summation notation, using an absolute value. In this case, using the value(%) command might not give you a closed form of the summation! As it turns out, evaluating total change as the limit of Riemann sums over the whole interval might not be the best way to go here. Don't worry if you can't get Maple to do what you want. You'll use the Int() command in a moment! value       Compute by using the Int() (or int() command.      In a couple sentances, describe how you could go about evaluating the total change of on the interval without needing to use the absolute value function.    Consider how the absolute value function acts on in intervals where the function is positive or negative.    "
...
},
{
  ...
  "number": "3.3.3.5",
  "title": "",
  "body": "  In this exercise, you will determine the error bounds for trapezoid and midpoint rules, since they both depend on the maximum value of over the interval. You may need to dig up some knowledge of critical values and extreme values on a closed interval in this exercise. integral approximation error      Plot over the interval and notice that the maximum value of occurs at a critical number of .      Find the critical number of by solving for . Evaluate at this -value to determine the value of that is used in formulas and .      Compute using formula .      Compute using formula .    "
},
{
...
},
{
  ...
  "number": "3.6.3.3",
  "title": "",
  "body": "  In this exercise, you will determine the value the limit analytically, using Maple to assist with methods that you would otherwise use on paper.   L'Hôpital's Rule states that if the limit is indeterminate of the form or , if and are differentiable at , and if near , then , assuming that this limit exists.      Try evaluating the integral using the int() command or Int() and value(%) commands. Notice how Maple evaluates the function, but not in terms of elementary functions.    There is no easy integration technique for this integral. You'll notice that Maple evaluates the integral in terms of the Elliptic F function, a transcendental function.      Luckily, you should not need to evaluate this integral in order to evaluate the limit. Instead, you may apply l'Hôpital's rule, so long as the limit is indeterminant of the form or . Which of these two indeterminant forms is applicable? limit l'Hôpital       Applying l'Hôpital's rule to the limit results in . Obviously in the denominator, but you will need to apply the Fundamental Theorem of Calculus for the numerator. Use the diff() command to evaluate and see how the Fundamental Theorem of Calculus is applied.      Using this result, determine on your own.    "
},
{
...
},
{
  ...
  "number": "3.7.3.1",
  "title": "",
  "body": "  In this exercise, you will create a function that gives the position of the shark at time , relative to its initial position.   The shark's distance function will contain an acceleration term for (where is the time you find in exercise ) but will not have an acceleration term after seconds.      Calculate by hand or using Maple how long it takes for the shark to reach its top speed of 13 m\/s if it accelerates at 5 m\/s . Assign this time to t1 . Note that the shark will no longer accelerate after seconds, since it has reached its top speed.      Define a piecewise function for the shark's velocity using the piecewise() command and assign this to sharkvelocity(t) . This velocity function should be linearly increasing for while the shark accelerates and then constant for . mathematical functions piecewise     You can read how to define piecewise functions in .    The shark's velocity function should look similar to the graph below.         Integrate with respect to to find the position function for the shark. Assign this piecewise position function to something like sharkposition .    "
},
{
...
},
{
  ...
  "number": "3.7.3.3",
  "title": "",
  "body": "  It's time to see if you can swim as far as the shore before the shark catches up to you!     Plot the position functions for you and the shark on the same graph. Be sure that your position graph includes your 50 m head start. Adjust your plot so that you can see the moment that the shark catches up to you.    You may wish to specify colours for each function so that you can tell which function is which.      Determine the time that the shark catches up to you. Solve for when the position function of the shark is equal to your position function. solving equations solve  solving equations fsolve     In this question, you are solving for time, . You can try using the solve() command first, then change it to fsolve() if necessary.      Evaluate the shark's position at this time and determine the position of the shark when it will catch you. Will you have make it to the shore safely?    "
},
{
...
},
{
  ...
  "number": "3.10.3.3",
  "title": "",
  "body": "  Consider the function .     Plot the graph of the function. Since is defined as a function of , you may require plotting the equation of the curve using implicitplot() .    You may need to include multiplication between and in the equation of this curve.      Determine the arc length of this curve for .    In this exercise, since is a function of , equation should be an integral in terms of .    "
},
{
...
},
{
  ...
  "body": " The Exponential Distribution   The probability density function for exponential distribution is defined as where is the mean and standard deviation of the probability distribution. probability exponential density function    "
},
{
...
},
{
  ...
  "number": "3.17.3.2",
  "title": "",
  "body": "  In this exercise, you will calculate several Taylor polynomial approximations for and determine the radius of convergence for the Taylor series approximation centred at .     Compute the nd, th, th, and rd degree Taylor polynomial approximations of centred at . Assign each to a new name.    Note that some powers may not appear in the polynomial if the coefficient for that term is equal to zero.      Plot the graph of and all four Taylor polynomial approximations on the same graph. How do the graphs of the Taylor polynomial approximations compare to as the degree increases?      In a new paragraph in your worksheet, state what you expect the radius of convergence to be for the Taylor series of centred at .    "
},
{
...
},
{
  ...
  "number": "4.8",
  "title": "",
  "body": "  Find the volume bounded by the curves and rotated about the -axis. volume of revolution         "
},
{
...
},
{
  ...
  "number": "4.11",
  "title": "",
  "body": "  After fish are introduced to a lake, the rate of growth for the population of fish is given by the differential equation , where is the population of fish after years. Define this differential equation in Maple. differential equations      Use dsolve() to give the solution to the differential equation for using the initial condition .           How many fish will there be after years (to the nearest fish)?           What does the population of fish approach after a long time? (Take the limit as tends to infinity or use a plot).         "
},
{
...
},
{
  ...
  "body": " Palettes Toolbar  You can open or close the palettes toolbar by clicking on the black arrows at the left side of the page.   Opening and closing the palettes toolbar.       With the palettes toolbar open, you can see several palettes that are available. These can be used to quickly access common operations and procedures by clicking on the appropriate buttons. Each palette can be expanded or closed. The Expression palette is especially useful for common functions.   Many common functions and operations can be in the Expression and Calculus pallettes.    "
},
{
...
},
{
  ...
  "number": "5.2",
  "title": "",
  "body": " Many common functions and operations can be in the Expression and Calculus pallettes.   "
},
{
...
},
{
  ...
  "body": " Plotting a Single Function  You can plot a function in Maple by using the plot() command. The only required parameter is the function you wish to plot. However, there are many additional parameters you can add to customise the way that the graph looks.   By default, Maple will plot trigonometric functions with an -axis from to .    plot( sin( cos(x) ) );    It is often useful to specify the interval for the -axis. Choose left and right endpoint for the -axis that are appropriate for your graph. plot axes intervals    If you are plotting a function of , then make sure to specify the interval as t=a..b .    plot(sin(cos(x)), x=-10..10);     The order of the parameters in the plot() command is important. The interval for the -axis must be listed before the interval for the -axis.   You can also specify the interval for the -axis for your graph. plot axes intervals   plot(sin(cos(x)), x=-10..10, y=-5..5);    "
},
{
