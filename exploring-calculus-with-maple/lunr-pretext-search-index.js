...
},
{
  ...
  "type": "Example",
  "number": "12.5",
  "title": "Visualizing the Derivative of <span class=\"process-math\">\\((x-1)^2\\sin(x)\\)<\/span>.",
  "body": " Visualizing the Derivative of  This example will illustrate how to use the interactive derivatives viewer to visualize the derivatives of on the interval .  First, you will need to start by typing out the function in the tutor. Unfortunately, you will not have access to the palettes toolbar in Maple, so you will need to type out commands such as sqrt() for square roots. You must also include the symbol * for multiplication.  You will also need to give an interval . Without the palettes toolbar, you will need to type Pi for the mathematical constant. Pi    The Derivatives tutor displays the given function as well as (and if the box is ticked). You can optionally specify the axes ranges as well as scaling.   "
},
{
...
},
{
  ...
  "body": " Differentiating using the Product, Power, and Cosine Rules  This example will use the Differentiation Methods tutor to show how various differentiation rules are applied to .  To begin, make sure that the function is typed in properly. You will not have access to the palettes toolbar in Maple, so you will need to type out commands such as sqrt() for square roots. You must also include the symbol * for multiplication.  You will also need to ensure that the correct variable is specified. Then you can click Start to begin.  If you are unsure which rule should be applied, you can ask for a hint.   You can click on an appropriate differentiation rule to see the specific rule applied, along with the resulting expression. If a rule cannot be applied, then the tutor will provide additional hints.   With the product rule applied, you can continue on to use the power and cosine rules.      "
},
{
...
},
{
  ...
  "body": " Approximating Area using Left-hand and Right-hand Rules  In this example, we will approximate the signed area between and the -axis using left-hand and right-hand rectangles. These are two common methods that we learn when first calculating Riemann sums. We will see how Maple can give the approximate area using a specific number of subintervals (which Maple calls a partition ).  To start, we will assign the function and calculate some approximations. mathematical functions exponential    f(x) := 10*exp(-x);      We will start by using eight subintervals and approximate the area using left-hand and right-hand rectangles. The method needs to be specified as well as the number of partitions. Maple will give the exact value of this Riemann sum, which can be converted to decimal using the evalf() command. evalf  ditto operator  integral approximation Riemann sum    with(Student[Calculus1]):  ApproximateInt(f(x), x = 0..4, method=left, output=value, partition=8); evalf(%);        ApproximateInt(f(x), x = 0..4, method=right, output=value, partition=8); evalf(%);       If we wish to get the actual area under the curve, then we can use an arbitrary rectangles and take the limit as . limit at infinity    In many cases, calculating the limit of this sum as is very numerically challenging and Maple may not output a numerical value. Using the value(%) command immediately after the limit() may help to convert the limit to a numerical result.    ApproximateInt(f(x), x = 0..4, method=left, output=sum, partition=n); limit(%, n = infinity); evalf(%);        "
},
{
  ...
  "type": "Example",
  "number": "14.3",
  "title": "Approximating the Area under <span class=\"process-math\">\\(x \\sin(x)\\)<\/span>.",
  "body": " Approximating the Area under  In this example, we will approximate the signed area between and the -axis using rectangle approximation. Unlike in the previous example, we will be using alternative methods for determining the height of the rectangle in each subinterval (or partition ), such as upper , lower , and midpoint .  We will start by assigning the function.   f(x) := x*sin(x);    mathematical functions sine    The method=upper option always uses the maximum value of in each subinterval for the height of each rectangle. Below is a plot of the rectangles as well as the approximation of area given by the method when using ten subintervals. As you can see, this method gives a very deliberate overestimate of the actual area bounded by this function and the -axis.   with(Student[Calculus1]):  ApproximateInt(f(x), x=-3..3, method=upper, output=plot, partition=10);     ApproximateInt(f(x), x=-3..3, method=upper, output=value, partition=10);      The method=lower option always uses the minimum value of in each subinterval for the height of each rectangle.   ApproximateInt(f(x), x=-3..3, method=lower, output=value, partition=10);      Finally, the midpoint rule uses the value of the function at the midpoint in each subinterval as the height of each rectangle. Below is a plot of twenty rectangles using the midpoint rule.   ApproximateInt(f(x), x=-3..3, method=midpoint, output=plot, partition=20);    "
},
{
...
},
{
  ...
  "type": "Example",
  "number": "15.10",
  "title": "Volume Obtained by Rotating a Region about a Vertical Axis.",
  "body": " Volume Obtained by Rotating a Region about a Vertical Axis  In this example, we will determine the volume of the solid obtained by revolving the region bounded by and about . Since this is the same region as described in , most of the steps are the same. However, in this case we must set the Line of Revolution information to be a vertical axis with an offset of from the -axis.   "
},
{
  "id": "sec_arc_length",
  "level": "1",
  ...
  "number": "15.5",
  "title": "Arc Length",
  "body": " Arc Length  arc length  We can use the ArcLength() command to find the arc length of a function over a specified interval. To use this command, we must load the Student[Calculus1] package. Packages are loaded using the with() command and it is typically a good idea to load any necessary packages at the top of your worksheet. Loading a package only needs to be done once per Maple worksheet, but needs to be run each time you open a new or previously closed document. packages Student[Calculus1]   We can use a single ArcLength() command to calculate the arc length for a simple function such as over an interval . It can be useful to assign the function first. evalf    f(x) := x^2;      The ArcLength() command will attempt to give the exact value of the arc length. Using evalf(%) allows us to display the result as a decimal.   with(Student[Calculus1]):  ArcLength(f(x), x=0..Pi); evalf(%);    Pi     Sometimes it may not be possible to express the exact value of arc length algebraically, such as in the case of the arc length of over the interval . mathematical functions sine    g(x) := sin(x);       arclen := ArcLength(g(x), x=0..Pi);       is a transcendental function that you don't have to know too much about. However, this is a numerical value that can be written as a decimal.   evalf(arclen);      To have a better understanding of why the exact value may not be able to be given algebraically, it can be useful to see what integral is involved to compute the arc length. To do this, we can add the parameter output = integral to the ArcLength() command. arc length output=integral    ArcLength(g(x), x=0..Pi, output=integral);      "
},
{
...
},
{
  ...
  "body": " Defining and Evaluating Series  We use the capitalized Sum() command to display the summation notation for a series symbolically. This also makes it possible to assign a name to the sum and use it in other commands later. Much like with sequences, we need an expression for a generic term in the series using an index, such as . We then specify the range for the index that generates the terms in that sequence that are to be summed. sequences and series Sum   For a simple example, we can add the five values from the sequence in . This is a finite series, since only finitely many terms are summed.   s1 := Sum(2*k^2-k, k=1..5);      We may use the value() command on the result of a capitalized Sum() command to display the result. Alternatively, the lowercase sum() command directly evaluates the value of the sum, without displaying the series. value  sequences and series sum    value(s1);       sum(2*k^2-k, k=1..5);      To determine the value of an infinite series, we can use infinity as a bound on the index. The value of an infinite sum may be a value (in the case of a convergent sum) or (in the case of a divergent sum). sequences and series infinite series    Sum((2\/3)^k, k=0..infinity); value(%);       This infinite series gives us a finite sum, so it is said to be convergent.   Sum(sin(k)+1, k=1..infinity); value(%);    mathematical functions sine     The infinite sum is said to be divergent.  "
},
{
...
},
{
  ...
  "body": " if \/ else and elif Statements  conditional statements if\/else statements  conditional statements if\/elif\/else statements   If and else statements allow us to test various conditions. The result then changes based on whether the given condition is true or false .  For example, let's say that someone makes the statement \"If it's sunny tomorrow, then I will go for a bike ride.\" If it is, in fact, sunny tomorrow, then that person will go for a bike ride. In other words, since the \"sunny tomorrow\" condition becomes TRUE, then the following implication \"go for a bike ride\" will happen.  As the statement stands now, we have no idea what this person will do if it is not sunny. Perhaps they will still go for a bike ride, perhaps not. We can define this action with an else statement.  In other words, we can define what will happen if it is not sunny . For example: \"If it's sunny tomorrow, then I will go for a bike ride. Otherwise, I will read a book at home.\"   From this example, we know that the following are true.  If it's sunny tomorrow, then I will go for a bike ride.  If it's not sunny tomorrow, then I read a book at home.     Conditional Based on Numerical Value  We will outline this concept with a silly example. We define two variables and . If is true (which it is because obviously is less than ), we print the word \"Mario\". Otherwise (if it's false ), we print the word \"Luigi\". Therefore, we expect that \"Mario\" will be printed.   You can use Shift+Enter to create multiple lines within the same Maple input and hit Enter to run them all at once.    a := 2: b := 5:  if a < b then print(\"Mario\") else print(\"Luigi\") end if        Conditional Based on Even\/Odd  Now we consider a more useful example. We define a function and check whether substituting into this function outputs an even number or an odd number.  The type() command is used here to check if the expression has the specified property. Maple recognizes a wide variety of different types, including even and odd .   f(x) := x^2 + 3*x - 4:  if type(f(2), even) then print(f(2), \"even\") else print(f(2), \"odd\") end if        Conditional Based on Finite\/Infinite  limit  In this example, we define a function and then use an if statement to verify whether or not the limit of as approaches is numeric . In other words, we are checking to see whether or not this limit exists (and is finite).   f(x) := 1\/x:  L := limit(f(x), x=0):  if type(L, numeric) then print(\"Limit is defined\") else print(\"Limit is undefined\") end if       The elif (else if) command allows us to add more than one condition to our statement. For example, if we want to test whether a particular number is positive, negative, or zero, and wish to have different outputs based on these three possibilities, we can do so with a combination of if , else , and elif .   Multiple Conditional Based on Numerical Value  In this example, we will use one of conditional statements to illustrate the first derivative test in calculus. We would like to answer the following question: for a particular value , is the function (i) increasing (positive derivative), (ii) decreasing (negative derivative), or (iii) neither increasing nor decreasing (zero derivative)?   g(x) := x^4 - 4*x^3 + 3*x^2:  a := 0:  if subs(x = a, diff(g(x), x)) > 0 then print(\"increasing\") elif subs(x = a, diff(g(x), x)) < 0 then print(\"decreasing\") else print(\"neither\") end if      This implies that is equal to .   "
},
{
  ...
  "body": " Conditional Based on Numerical Value  We will outline this concept with a silly example. We define two variables and . If is true (which it is because obviously is less than ), we print the word \"Mario\". Otherwise (if it's false ), we print the word \"Luigi\". Therefore, we expect that \"Mario\" will be printed.   You can use Shift+Enter to create multiple lines within the same Maple input and hit Enter to run them all at once.    a := 2: b := 5:  if a < b then print(\"Mario\") else print(\"Luigi\") end if      "
},
{
...
},
{
  ...
  "body": " Multiple Conditional Based on Numerical Value  In this example, we will use one of conditional statements to illustrate the first derivative test in calculus. We would like to answer the following question: for a particular value , is the function (i) increasing (positive derivative), (ii) decreasing (negative derivative), or (iii) neither increasing nor decreasing (zero derivative)?   g(x) := x^4 - 4*x^3 + 3*x^2:  a := 0:  if subs(x = a, diff(g(x), x)) > 0 then print(\"increasing\") elif subs(x = a, diff(g(x), x)) < 0 then print(\"decreasing\") else print(\"neither\") end if      This implies that is equal to .  "
},
{
...
},
{
  ...
  "number": "A.12",
  "title": "",
  "body": " In this example, the Int() command and int() command behave differently. The Int() command outputs the integral itself, whereas the int() command immediately computes the result of the integral.   Using Int() followed by value(%) is a good way to display the integral and then evaluate it.    Int(2*x, x = 10 .. 13);       int(2*x, x = 10 .. 13);      "
},
{
